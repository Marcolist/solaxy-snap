 ...
 * 1. localhost (HTTP) - Development only
 * 2. solaxy.io domains (hardcoded) - Official apps, audit-covered
 * 3. User-approved domains - Dynamic approval for partners/ecosystem
 *
...
async function isAllowedOrigin(origin) {
  try {
    ...
    // Tier 1: Development
    if (protocol === 'http:' && normalizedHostname === 'localhost') {
      return true;
    }

    // Require HTTPS for all production domains
    if (protocol !== 'https:') return false;

    // Tier 2: Hardcoded trusted domains (audit-covered)
    if (isHardcodedTrusted(normalizedHostname)) {
      return true;
    }

    // Tier 3: User-approved domains (dynamic)
